...
# batches -- rather than one at a time -- reclaims the wall-clock time spent waiting on
# each response before the next candidate's slot is even requested, without abandoning
# the early-stop-once-max_valid-is-reached optimization (which a single all-at-once
# gather() would defeat, since in-flight calls can't be un-sent).
METADATA_VALIDATION_BATCH_SIZE = 4
