...
def test_prompt_generation_uses_lastfm_as_ai_context_without_fixed_quota(monkeypatch) -> None:
    ...
    async def fake_discover(anchors, *, limit=40, max_anchors=3):
        assert anchors == first_draft["tracks"]
        assert limit >= 20
    ...
    assert result["lastfm"] == {
        ...
        "suggestions": 2,
        "selected": 1,
        "strategies": ["similar_artist", "similar_track"],
    }
...
def test_seed_context_skips_prompt_anchor_discovery(monkeypatch) -> None:
    ...
    token = main_module._SEED_RECOMMENDATIONS.set(tuple(seed_signals))
    try:
        result = asyncio.run(
            main_module._generate(
        ...
        main_module._SEED_RECOMMENDATIONS.reset(token)

    assert calls == 2
    assert result["lastfm"]["selected"] == 1
    assert result["tracks"][0]["lastfm_strategy"] == "similar_artist"
