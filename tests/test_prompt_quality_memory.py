...
from backend.creative_intent import EVALUATE_SYSTEM_PROMPT, INTERPRET_SYSTEM_PROMPT
...
def test_explicit_soft_curation_intent_is_playlist_wide_and_generalized() -> None:
    ...
    semantic_guard = (INTERPRET_SYSTEM_PROMPT + EVALUATE_SYSTEM_PROMPT).casefold()
    ...
    assert "playlist-wide creative requirements" in semantic_guard
    assert "subjective taste" in semantic_guard
    assert "party" not in guidance
    assert "festive" not in guidance
    assert "party" not in semantic_guard
    assert "festive" not in semantic_guard
...
def test_latest_feedback_case_activates_temporal_and_live_guards() -> None:
    cases = {
        case.id: case
        for case in approved_prompt_quality_cases(flow="generation")
    }
    case = cases["generation-open-range-and-live-filter-it"]
    ...
    ) == (case.expectations["open_ended_year_from"], current_year)
    assert case.expectations["open_ended_year_to"] == "present"
    assert track_matches_variant(
        {
            "artist": "Example Artist",
            "title": f"Example ({case.expectations['compact_live_marker']})",
        },
        "live",
    )
